...
final class CreateAccountViewModelTest: XCTestCase {
    ...
    private let passwordConfirmTextfieldInput = PublishSubject<String?>()
    ...
    override func setUp() {
        ...
        let output = sut.transform(input: CreateAccountViewModelInput(
                                    mail: emailTextfieldInput
                                        .asDriver(onErrorDriveWith: .empty()),
                                    password: passwordTextfieldInput
                                        .asDriver(onErrorDriveWith: .empty()),
                                    passwordConfirm: passwordConfirmTextfieldInput
                                        .asDriver(onErrorDriveWith: .empty()),
                                    createButtonClick: createButtonInput
                                        .asDriver(onErrorDriveWith: .empty()),
                                    backButtonClick: backButtonInput
                                        .asDriver(onErrorDriveWith: .empty()),
                                    viewTap: viewTapInput
                                        .asDriver(onErrorDriveWith: .empty()),
                                    keyboardNotifications: keyboardNotifications
                                        .asDriver(onErrorDriveWith: .empty())))

        disposeBag.insert(output.triggers.drive(),
        output.isLoading.drive(),
        output.areCredentialsValid.drive(),
    ...
    func testBackButtonClickedSuccessfully() {
        scheduler
    ...
    func testCredentialsValidationCalled() {
        ...
        scheduler
            .createColdObservable([.next(0, "qwertyqwerty")])
            .bind(to: passwordConfirmTextfieldInput)
            .disposed(by: disposeBag)
        scheduler
